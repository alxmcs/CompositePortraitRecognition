...
cursor.execute(
    "CREATE TABLE person(id integer PRIMARY KEY, name text, patronymic text, surname text, comment text, date_added "
    "date)")
...
cursor.execute(
    "CREATE TABLE embedding(id integer PRIMARY KEY, value varbinary(100), date_added date, model_id INTEGER NOT NULL, "
    "FOREIGN KEY (model_id) REFERENCES model(id), person_id INTEGER NOT NULL, FOREIGN KEY (person_id) REFERENCES "
    "person(id), "
    "preprocessing_id INTEGER NOT NULL, FOREIGN KEY(preprocessing_id) REFERENCES preprocessing(id)")
